(function ($)
{
  ...
  {
    var ErrorMessage = {
      DATETIME: 'Input must be in valid datetime format',
      DATETIMELOCAL: 'Input must be in valid local time format',
      DATE: 'Input must be in YYYY-MM-DD format',
      MONTH: 'Input must be in YYYY-MM format',
      TIME: 'Input must be in HH:MM:SS or HH:MM:SS.nn format',
      WEEK: 'Input must be in YYYY-WW format',
      NUMBER: 'Input must be a valid number',
      RANGE: 'Input must be a valid number',
      EMAIL: 'This should be a valid email address (eg. john.smith@domain.com)',
      URL: 'Input must be a valid URL',
      TEL: 'Phone numbers may only contain numbers',
      REQUIRED_FIELD: 'This field is required',
      INVALID_INPUT: 'Invalid input',
      RANGE_BETWEEN: 'Must be between %min and %max',
      RANGE_MIN: 'Must be greater than %min',
      RANGE_MAX: 'Must be less than %max'
    };

    if (typeof LocalizedInlineValidationErrorMessage !== 'undefined') {
      $.extend(ErrorMessage, LocalizedInlineValidationErrorMessage);
    }
    ...
    var __oPredefValid = {
      ...
      datetimelocal: {
        ...
        sValid: ErrorMessage.DATETIMELOCAL,
        bCaseSensitive: true
      },
      date: {
      ...
      week: {
        ...
        sValid: ErrorMessage.WEEK
      },
      number: {
        rValidate: /^\-?\d+(\.\d+)?$/,
        sValid: ErrorMessage.NUMBER
      },
      range: {
        rValidate: /^\-?\d+(\.\d+)?$/,
        sValid: ErrorMessage.RANGE
      },
      email: {
        // regex taken from rfc3696
        rValidate: /^[a-z0-9!$'*+\-_&]+(\.[a-z0-9!$'*+\-_&]+)*@([a-z0-9]+(-+[a-z0-9]+)*\.)+([a-z]{2,6})$/,
        sValid: ErrorMessage.EMAIL
      },
      url: {
        rValidate: /^https?:\/\//,
        sValid: ErrorMessage.URL
      },
      tel: {
        rValidate: /^\+?([\d\(\)\-x\s]+)$/,
        sValid: ErrorMessage.TEL
      }
    },
    opt = $.extend(true, {
      sRequired: ErrorMessage.REQUIRED_FIELD,
      sValid: ErrorMessage.INVALID_INPUT,
      sParentSelector: null,
      sParentErrorClass: 'error',
      sMsgMarkup: '<em class="error"/>',
      asRangeMsg: {
        sRange: ErrorMessage.RANGE_BETWEEN,
        sMin: ErrorMessage.RANGE_MIN,
        sMax: ErrorMessage.RANGE_MAX
      }
    }, (arguments.length === 1 ? arguments[0] : {}));
  ...
  $.fn.iframeSwitcher = function (options) {
    ...
    return this.each(function () {
      ...
      self.on('change', function (e) {
        ...
        if (typeof checkout_rec !== "undefined" && checkout_rec == 'true') {
